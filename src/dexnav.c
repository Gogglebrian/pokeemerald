...
static void Task_DexNavSearch(u8 taskId)
{
	...
	/* This disables the scaring 'em away mechanic
...
static u8 DexNavGetAbilityNum(u16 species, u8 searchLevel)
{
	...
	/* With no hidden abilities implemented, we can skip this entire thing
    if (searchLevel < 5)
    {
...
static void TryDrawIconInSlot(u16 species, s16 x, s16 y)
{
    if (species == SPECIES_NONE || species > NUM_SPECIES)
        ...
        CreateBlackMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0xFFFFFFFF, 0); //silhouette
	else
...
static void DrawSpeciesIcons(void)
{
    ...
    for (i = 0; i < HIDDEN_WILD_COUNT; i++)
    {
        ...
        y = ROW_HIDDEN_ICON_Y;
        if (FlagGet(FLAG_SYS_POKEDEX_GET)) // Override detector mode
            TryDrawIconInSlot(species, x, y);
...
bool8 TryFindHiddenPokemon(void)
{
	...
	//overriding detector mode again
    if (!FlagGet(FLAG_SYS_POKEDEX_GET) || FlagGet(FLAG_SYS_DEXNAV_SEARCH) || GetFlashLevel() > 0)
    {
        (*stepPtr) = 0;
        return FALSE;
    }
