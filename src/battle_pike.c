...
static bool8 DoesTypePreventStatus(u16 species, u32 status)
{
    ...
    {
    case STATUS1_TOXIC_POISON:
        if (gBaseStats[species].types[0] == TYPE_STEEL || gBaseStats[species].types[0] == TYPE_POISON
            || gBaseStats[species].types[1] == TYPE_STEEL || gBaseStats[species].types[1] == TYPE_POISON)
            ret = TRUE;
        break;
    case STATUS1_FREEZE:
        if (gBaseStats[species].types[0] == TYPE_ICE || gBaseStats[species].types[1] == TYPE_ICE)
            ret = TRUE;
        break;
    case STATUS1_PARALYSIS:
        if (gBaseStats[species].types[0] == TYPE_GROUND || gBaseStats[species].types[0] == TYPE_ELECTRIC
            || gBaseStats[species].types[1] == TYPE_GROUND || gBaseStats[species].types[1] == TYPE_ELECTRIC)
            ret = TRUE;
        break;
    case STATUS1_BURN:
        if (gBaseStats[species].types[0] == TYPE_FIRE || gBaseStats[species].types[1] == TYPE_FIRE)
            ret = TRUE;
        break;
    case STATUS1_SLEEP:
        break;
...
static bool8 CanEncounterWildMon(u8 enemyMonLevel)
{
    if (!GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
        ...
        if (monAbility == ABILITY_KEEN_EYE || monAbility == ABILITY_INTIMIDATE || monAbility == ABILITY_TERRIFLYING)
        {
