...
#define MAX_RELEARNER_MOVES max(MAX_LEVEL_UP_MOVES, 64)
...
//Move Tutor ---------------------------------------------------
static void CB2_TutorMovesMain(void);
static void DoMoveTutorMain(void);
static void CreateTutorMovesList(void);
static void Task_WaitForFadeOutTutor(u8 taskId);
static void CB2_InitMoveTutor(void);
static void CB2_InitLearnTutorMoveReturnFromSelectMove(void);
...
static void DoMoveTutorMain(void)
{
    ...
    case MENU_STATE_SHOW_MOVE_SUMMARY_SCREEN:
        if (!gPaletteFade.active)
        {
            ShowSelectMovePokemonSummaryScreen(gPlayerParty, sMoveRelearnerStruct->partyMon, gPlayerPartyCount - 1, CB2_InitLearnTutorMoveReturnFromSelectMove, GetCurrentSelectedMove());
            FreeMoveRelearnerResources();
        }
        break;
    case 21:
...
//Move Tutor --------------------------------------------------
void TeachTutorMove(void)
{
    ScriptContext_Enable();
    CreateTask(Task_WaitForFadeOutTutor, 10);
    // Fade to black
    BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 0x10, RGB_BLACK);
}
...
static void Task_WaitForFadeOutTutor(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        SetMainCallback2(CB2_InitMoveTutor);
        gFieldCallback = FieldCB_ContinueScriptHandleMusic;
        DestroyTask(taskId);
    }
}
...
static void CB2_InitMoveTutor(void)
{
    ...
    sMoveRelearnerStruct->moveListMenuTask = ListMenuInit(&gMultiuseListMenuTemplate, sMoveRelearnerMenuSate.listOffset, sMoveRelearnerMenuSate.listRow);
    FillPalette(RGB_BLACK, 0, 2);
    SetMainCallback2(CB2_TutorMovesMain);
}
